...
class Site_Builder_Ajax_Handler {
    ...
    public function process_batch(): void {
        ...
        // P0 — offset comes from the DB (processed_count column), not from
        // the client. This is the key to resume-after-504: even if the tab
        // crashes and comes back with no state, the server knows exactly where
        // the last batch finished. The client-supplied offset is accepted only
        // as a hint for logging — never used as source of truth.
        $offset = (int)$import->processed_count;
        ...
        // If the lock had expired (5+ min without heartbeat) and got cleaned
        // up but the import row is still 'running' — restore the lock so
        // subsequent batches keep the heartbeat alive and no parallel import
        // can start.
        if ($tracker->get_lock() === null) {
            $tracker->acquire_lock($import_id, get_current_user_id());
        }
        ...
        // P1 — time-boxed batches. Hostinger (and most nginx-based shared
        // hosts) enforce a hard proxy_read_timeout of 60 seconds. If a batch
        // runs longer, nginx returns 504 while PHP keeps working, but the
        // client loses state. We stop the batch loop 45 seconds in — well
        // under the timeout, giving 15s headroom for response serialization
        // and jsonencode overhead. Filter allows advanced ops to tune per host.
        $max_batch_time = (float)apply_filters('sb_max_batch_seconds', 45);
        $batch_started  = microtime(true);
        // P2 — slow-task threshold. Tasks slower than this get logged as a
        // notice so the operator sees which pages are the accumulation
        // bottleneck (typically fsr_page with many images).
        ...
        foreach ($batch as $task) {
            ...
            // all hook side effects (menu-sync, cache invalidation, etc).
            $task_duration = microtime(true) - $task_started;
            if ($task_duration > $slow_task_threshold) {
                $slow_ctx = [
                    'kind'        => $kind,
                    'duration_ms' => (int)($task_duration * 1000),
                ];
                if (isset($task['data']['slug'])) {
                    $slow_ctx['slug'] = (string)$task['data']['slug'];
                }
            ...
            // P1 — abort the batch loop if we're approaching nginx timeout.
            // Client sees partial progress and can continue via the "Resume"
            // banner (P0) or the JS auto-continues if the tab is still open.
            if ((microtime(true) - $batch_started) >= $max_batch_time) {
                break;
            }
        }
        ...
        // cap (P1). This is normal, not an error — the client just continues
        // immediately with the next batch. Distinguished from "done" so the
        // UI doesn't show "complete" after a partial batch.
        $batch_elapsed = microtime(true) - $batch_started;
        $time_boxed = !$done && ($batch_elapsed >= $max_batch_time) && ($processed_in_batch < count($batch));
        ...
        wp_send_json_success([
            ...
            'batch_ms'      => (int)($batch_elapsed * 1000),
        ]);
    }
     ...
     * refreshed, losing client-side state. When the operator comes back, we
     * detect the stranded import and offer to continue it from the last
     * server-side offset (processed_count column, incremented after every batch).
     *
     * Returns { active: true, import: {...} } when there IS a resumable
     * import, or { active: false } otherwise. Never returns an error — a
     * missing/expired lock is not an error state.
     */
    public function check_active_import(): void {
        $this->authorize();

        // Look for any import row still marked 'running' in the DB. This is
        // more reliable than get_lock() alone — the lock option expires after
        // 5 minutes of no heartbeat (stale-lock cleanup), but the DB row keeps
        // the actual state. Without this we'd fail to surface imports the
        // operator abandoned overnight and returned to the next morning.
        global $wpdb;
        $tracker = new Site_Builder_Import_Tracker();
        $imports_table = $wpdb->prefix . 'site_builder_imports';
        $import_id = (int)$wpdb->get_var(
            "SELECT id FROM $imports_table WHERE status = 'running' ORDER BY id DESC LIMIT 1"
        );
        ...
        // Guard: if the DB says everything's done but status is still running,
        // finish it now. Prevents an empty resume that just marks it complete.
